...
This codebook demonstrates how fundamental data on Thai stocks from the [Gurufocus](http://gurufocus.com) and SET TRI from [Stock Exchange of Thailand](http://set.or.th) are transformed into the tidy format used in this [analysis](index.html).

# Getting and Cleaning Data
## Thai Stock Data
The data on Thai stock fundamentals are obtained from [Gurufocus API](http://gurufocus.com). The symbols associated wit the stock market was obtained by using a filter on the website and saved to ```symbols.csv```. We download JSON associated with this and save them to the ```stocks``` folder.
```{r, eval=FALSE}
library(jsonlite)
library(httr)
...
The dataset includes annual fundamental data of 463 Thai stocks from 2002-2015. The variables are divided into 7 groups same as the financial statement format of [Gurufocus](http://gurufocus.com) namely:
