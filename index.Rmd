...
Testing reveals out-of-sample 64.72% accuracy. We also get 54.76% out-of-sample sensitivity, meaning 54.76% of stocks we labelled as market beaters are actually so.
...
```{r}
mean(backtest[backtest$winloss==pred,]$priceg)
```
...
This gives a slightly inferior out-of-sample accuracy of 63.13% and a much worse sensitivity of 43.54% respectively.
...
This gives the best result so far with 85.13% accuracy and 79.25% sensitivity.
...
All our models has a better than random accuracy and sensitivity (except for logit boost). They are also shown to perform reasonably in the backtest (20.19%, 14.60% and 20.08% respectively). Random forest has the highest accuracy and sensitivity; nonetheless it performs worse than support vector machine in the backtest. This is because our models have several limitations:

* We predict if a stock 'beats the market' but not 'by how much' so the discrepancy in accuracy/sensitivity and backtest performances between random forest and svm.
